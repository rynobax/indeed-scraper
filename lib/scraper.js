...
function parseJobList(body){
  ...
  const filtered = jobs.filter((i, e) => {
    const job = $(e);
    return !job.children().hasClass('sdn');
  })
  ...
  const jobObjects = filtered.map((i, e) => {
    ...
    const dateString = job.find('.date').text().trim().subString(0, indexOf(' ago')+4);

    const date = chrono.parse(dateString.replace('0+', '1'))[0].ref;
