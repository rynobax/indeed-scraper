...
function Query(qo) {
  ...
  this.excludeKeyWords = qo.excludeKeyWords || [];
  this.includeKeyWords = qo.includeKeywords || [];
...
Query.prototype.url = function() {
  ...
  q += "?q=" + this.query;
  q += "&l=" + this.city;
  q += "&as_any" + this.includeKeyWords.join("%20"),
  q += "&as_not" + this.excludeKeyWords.join("%20"),
...
function parseJobList(body, host, excludeSponsored) {
  ...
  const jobObjects = filtered
    .map((i, e) => {
      ...
      const url = "https://" + host + job.attr("href");
      ...
      return {
        ...
        company: company,
        location: location,
        originalJobLink: originalJobLink,
        postDate: postDate,
        salary: salary,
